...
function playRound(player) {
  ...
  const done =
    playerWins / totalRounds > 0.5 || computerWins / totalRounds > 0.5;
  ...
  const finishGameButton = document.querySelector(
    ".result__buttons .finishGame"
  );
...
function finishGame() {
  ...
  const element = el(
    "li",
    `${win ? "Þú vannst " : "Tölva vann "} ${playerWins}–${computerWins}`
  );
